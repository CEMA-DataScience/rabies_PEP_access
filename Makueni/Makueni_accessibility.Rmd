...
mkn.shp <- malariaAtlas::getShp(ISO = "KEN", admin_level = "admin1", version = "202403")

analysis.shp <- mkn.shp[mkn.shp$name_1=="Makueni",]
analysis.shp1 <- mkn.shp[mkn.shp$name_1=="Makueni",]
...
# Assuming `friction` is already a SpatRaster
# Convert SpatRaster to RasterLayer
friction_raster <- raster(friction)
friction_raster1 <- raster(own_friction)
...
T2 <- gdistance::transition(friction_raster1, function(x) 1/mean(x), 8) 
...
analysis.shp <- as(analysis.shp, "Spatial")
...
all_facilities_access_raster <- rast(all_facilities_access_raster)
all_facilities_access_raster1 <- rast(all_facilities_access_raster1)


# Convert SpatialPolygonsDataFrame to sf
analysis.shp <- st_as_sf(analysis.shp)

p <- malariaAtlas::autoplot_MAPraster(
  all_facilities_access_raster, 
  shp_df=analysis.shp, 
  printed=F)

p1 <- malariaAtlas::autoplot_MAPraster(
  all_facilities_access_raster1, 
...
barchart_plot <- ggplot(bar_adm0, aes(x = reorder(travel_minutes,-all_facilities_access_raster),
  ...
  theme_bw() +
  labs(title="Travel minutes to nearest 
       facility given population",
    x = "", y = "") +
  theme_classic() +
...
ggsave("combined_driving.png",figure, width = 20, height = 12, units = "in", bg="white")
```
...
figure <-ggarrange(combined_plots3, combined_plots4,
    labels = c("C", "D"), nrow  = 1, font.label=list(color="black",size=25,  align = "hv"))

figure

all_figs <- ggarrange(combined_plots3, combined_plots4,
                      combined_plots1, combined_plots2,
                      labels = c("A", "B", "C", "D"), nrow = 2,
                      ncol = 2,
                      font.label=list(color="black",size=25,  align = "hv"))

ggsave("combined_walking.png",figure, width = 20, height = 12, units = "in", bg = "white")

ggsave("combined_figure.png",all_figs, width = 20, height = 12, units = "in", bg = "white")
...
pep_fac_CT_combined <- CT_combined |>
  filter(place_to %in% c("Kibwezi Sub County Hospital", "Kilungu Sub County Hospital", "Kisau Sub County Hospital", "Makindu Sub County Hospital", "Makueni County Referral Hospital", "Matiliku Sub County Hospital", "Mbooni Sub County Hospital", "Mtito Andei Sub County Hospital", "Mukuyuni Sub County Hospital", "Sultan Hamud Sub County Hospital", "Kambu Sub County Hospital", "Tawa Sub County Hospial"))

```

# buiding the main function for all facilities
...
df <- CT_combined %>%
  ...
  speed_kph = c(40))  # Speed in kilometers per hour - we adopted Weiss et al speeds and since most roads in Makueni are uncategorised we adopted 40km/hr
...
results <- result_df %>%
  dplyr::select(- c( latitude_from,longitude_from, elevation,longitude_to, latitude_to)) %>%
...
# buiding the main function for pep facilities

```{r}

# Sample data frame with latitude and longitude for multiple origins and destinations
df_pep <- pep_fac_CT_combined %>%
  mutate(
  origin_lon = longitude_from,
  origin_lat = latitude_from,
  dest_lon = longitude_to,
...
# Create a matrix to store results
results_matrix <- matrix(NA, nrow = nrow(df_pep), ncol = 2)

# Loop through each row to calculate distance and time
for (i in 1:nrow(df_pep)) {
  results_matrix[i, ] <- calculate_distance_time(df_pep$origin_lon[i], df_pep$origin_lat[i], df_pep$dest_lon[i], df_pep$dest_lat[i], df_pep$speed_kph[i])
}

# Combine the results with the original data frame
result_df_pep <- cbind(df_pep, results_matrix)

# Rename columns for clarity
colnames(result_df_pep) <- c("subcounty", "ward", "latitude_from", "longitude_from", "elevation", "place_from", "ID", "place_to", "latitude_to", "longitude_to","origin_lon", "origin_lat", "dest_lon", "dest_lat", "speed_kph", "distance", "time_hours")

# Print the result
print(result_df_pep)

results <- result_df_pep %>%
  dplyr::select(- c( latitude_from,longitude_from, elevation,longitude_to, latitude_to)) %>%
  mutate("time(minutes)" = time_hours*60) %>%
  mutate("time(mins)_category" = ifelse(`time(minutes)` <10, "<10",
                                 ifelse(`time(minutes)` >10 & `time(minutes)` <20, "10 - 20",
...
time_graph_pep <- ggplot(props, aes(x = `time(mins)_category`, y = proportion, label = scales::percent(proportion)))+
  geom_col(fill = "#611dc0")+ 
  theme_bw()+
  labs(x = "time (minutes) category", y = "% proportion")+
  geom_text(position = position_dodge(width = .9),
              vjust = -0.8,
...
ggsave("time_graph_pep.png",plot = time_graph_pep,
       path = "/Users/mumbuamutunga/Library/CloudStorage/Dropbox/Datasets/Makueni/maps",
       width = 23.5, height = 16, units = "cm", dpi = 600)

fig_time_travel <-ggarrange(time_graph, time_graph_pep,
    labels = c("A", "B"), nrow  = 1, font.label=list(color="black",size=20,  align = "hv"))

fig_time_travel

ggsave("fig_time_travel.png",plot = fig_time_travel,
       path = "/Users/mumbuamutunga/Library/CloudStorage/Dropbox/Datasets/Makueni/maps",
       width = 23.5, height = 16, units = "cm", dpi = 600)
