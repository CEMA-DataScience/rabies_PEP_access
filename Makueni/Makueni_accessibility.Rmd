...
#write_csv(contact_tracing , "contact_tracing_clean.csv")

# coordinates(contact_tracing) <- ~longitude + latitude
# 
# proj4string(contact_tracing) <- CRS("+init=epsg:4326")
# 
# contact_tracing <- spTransform(contact_tracing,CRS("+proj=longlat +datum=WGS84 +no_defs"))
# 
# sp::points2grid(contact_tracing, tolerance = 0.9)
# 
# #extent <- ("37.141288758,38.519416809,-2.992213250,-1.515167237 [EPSG:4326]")
# 
# r <- raster(contact_tracing)
# r <- projectRaster(r, crs=4326)
...
calculate_distance <- function(lat1, lon1, lat2, lon2) {
  coords1 <- c(lon1, lat1)
  coords2 <- c(lon2, lat2)
  distance <- distVincentySphere(coords1, coords2)
  return(distance)
}

calculate_time <- function(distance, speed) {
  time <- distance / speed
  return(time)
}

# Function to calculate time based on distance and speed
origins <- data.frame(lat = c(contact_tracing$latitude_from), 
                    lon = c(contact_tracing$longitude_from))

# Makueni County Referral Hospital (destination) coordinates
destination <- c(-1.78363, 37.62843)

#assume driving with a constant speed of 50
speed <- 50

# Create an empty data frame to store the results
results <- data.frame(
  Distance_meters = numeric(0),
  Distance_kilometers = numeric(0),
  Time_seconds = numeric(0),
  Time_minutes = numeric(0)
)

# Calculate time and distance for each starting point
for (i in 1:nrow(contact_tracing)) {
  lat1 <- contact_tracing$latitude_from[i]
  lon1 <- contact_tracing$longitude_from[i]

  # Calculate distance in meters
  distance <- calculate_distance(lat1, lon1, contact_tracing$latitude_to[1], contact_tracing$longitude_to[2])
  cat("Distance from starting point", i, "to destination:", distance, "meters\n")

  # Convert distance to kilometers
  distance_km <- distance / 1000
  cat("Distance from starting point", i, "to destination:", distance_km, "kilometers\n")

  # Calculate time in hours
  time_seconds <- calculate_time(distance, speed)
  cat("Time to travel from starting point", i, "to destination:", time_hours, "hours\n")

  # Convert time to minutes
  time_minutes <- time_seconds / 60
  cat("Time to travel from starting point", i, "to destination:", time_minutes, "minutes\n\n")
